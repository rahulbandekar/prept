...
export const checkUser = async () => {
  ...
  try {
    ...
    console.log("🔍 Debug - PLAN_CREDITS credits:", credits);

    const loggedInUser = await db.user.findUnique({
      where: { clerkUserId: user.id },
    });

    console.log("🔍 Debug - loggedInUser from DB:", loggedInUser);

    if (loggedInUser) {
      // Interviewers don't have a credit subscription — skip allocation
      if (loggedInUser.role === "INTERVIEWER") return loggedInUser;
      console.log("✅ Returning INTERVIEWER with creditBalance:", loggedInUser.creditBalance);
      ...
      if (shouldAllocateCredits(loggedInUser, currentPlan)) {
        ...
        console.log("🔄 Allocating new credits - rolledCredits:", rolledCredits);
    ...
    console.log("✨ Creating new user with credits:", credits);
